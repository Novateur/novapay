<?php
	...
	if($query->rowCount()>0)
	{
	  ...
	  echo "<div class='col-sm-4'>";
		...
		foreach($query as $r)
		{
			$hashed_id = base64_encode($r['id']);
			$md5_id = md5($r['id']);
			echo"<tr>
			<td style='font-size:15px'>{$r['day']} ".strtoupper($r['month']).",{$r['year']} @ {$r['time1']}</td>
			<td>";if($r['status']==0 && $r['user_id']==$id){ echo "<a href='confirm.php?ref={$hashed_id}&ref_id={$md5_id}'><i class='i i-publish'></i></a>";}else{ echo "";}echo "</td>
