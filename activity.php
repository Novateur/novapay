...
<?php echo "<body onload=\"get_cards_trasact()\">"; ?>
      ...
      <ul class="nav navbar-nav hidden-xs nav-user user pull-right">
		...
		<li>
          ...
          <ul class="dropdown-menu animated fadeInRight">            
			...
			<li>
              <a href="#" onclick="confirm_payment()"><i class="fa fa-check-square"></i> confirm transaction</a>
            </li>
            <li class="divider"></li>
            <li>
                ...
                <section class="scrollable padder"><br/> 
					<div class="container">
						<div class="container">
							<div class="col-sm-12">
								<?php
									if(isset($_GET['empty_term']))
									{
										echo "<div class='alert alert-danger'>The search field cannot be empty,input a transaction name to search</div>";
									}
								?>
							</div>
  ...
  <script>
	...
	{
		$.get("handler/get_activity.php?page=1",function(response){
	...
	{
		$.get("handler/get_activity.php?page="+page_to,function(response){
			$('#paste_transactions').html(response);
		})
	}	
	function paginate_simple(page_to)
	{
		$.get("handler/get_simple_activity.php?page="+page_to,function(response){
			$('#paste_transactions').html(response);
		})
	}	
	function get_simple_activity()
	{
		$.get("handler/get_simple_activity.php?page=1",function(response){
			$('#paste_transactions').html(response);
		})
	}	
	function get_detailed_activity()
	{
		get_cards_trasact();
	}
